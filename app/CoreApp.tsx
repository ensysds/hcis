...
type CoreModule = { key: string; label: string; name: string; description: string | null; abilities: string[] };
type CoreWorkspace = {
  modules: CoreModule[];
  summary: {
    attendance: { check_in_at: string | null; check_out_at: string | null } | null;
    leave_remaining: number;
    latest_payslip: { period: string | null; status: string } | null;
  };
};
...
const services = [
  { module: "leave", title: "Ajukan cuti", note: "Saldo dan riwayat cuti", icon: Plane, tone: "mint" },
  { module: "attendance", title: "Koreksi absen", note: "Riwayat kehadiran", icon: TimerReset, tone: "sky" },
  ...
  { module: "knowledge", title: "Knowledge", note: "Artikel dan knowledge sharing", icon: FileText, tone: "blue" },
  { module: "innovation", title: "Innovation", note: "Kirim dan pantau ide", icon: MessageSquareText, tone: "amber" },
  { module: "performance", title: "Performance", note: "KPI dan appraisal", icon: CircleDollarSign, tone: "mint" },
];

const activities = [
  { module: "attendance", day: "17", month: "JUL", title: "Kehadiran hari ini", detail: "Masuk 08.03 · Menunggu check-out", status: "Berjalan", kind: "active" },
  { module: "attendance", day: "16", month: "JUL", title: "Kehadiran kantor", detail: "08.01 — 17.08 · 9j 07m", status: "Tepat waktu", kind: "success" },
  { module: "attendance", day: "15", month: "JUL", title: "Kehadiran kantor", detail: "08.12 — 17.05 · 8j 53m", status: "Terlambat 12m", kind: "warning" },
];
...
export function CoreApp() {
  ...
  useEffect(() => {
    ...
    fetch("/api/hcis/bootstrap", { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        const payload = await response.json();
        if (!response.ok) throw new Error(payload.message ?? "Akses HCIS tidak dapat dimuat.");
        return payload as CoreWorkspace;
      })
      .then((payload) => {
  ...
  const moduleKeys = useMemo(() => new Set(workspace?.modules.map((module) => module.key) ?? user?.modules ?? []), [workspace, user]);
  const availableServices = useMemo(() => services.filter((service) => moduleKeys.has(service.module)), [moduleKeys]);
  const visibleNavItems = useMemo(() => navItems.filter((item) => item.key !== "persetujuan" || user?.roles.some((role) => ["manager", "general_manager", "director"].includes(role))), [user]);
  ...
  const finishAttendance = async () => {
    try {
      ...
      const response = await fetch("/api/hcis/attendance", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.message ?? "Kehadiran gagal direkam.");
